...
**物品统一走"完全转换"**（作者 2026-09-13 定的规则：只有性质变，外观一律不变）：

| 项 | 规则 |
|---|---|
| **外观** | **永远保持源物品的样子**（客户端 mixin 把模型画回去） |
| **本体** | 真的换成名字所指的物品 —— 于是"能合成钻石装备、能进信标、不能再当方块放下"等全部性质自动成立 |
| **耐久** | 目标本身有耐久才需要动它：取**源物品原本的耐久**；源物品没有耐久（石头/木棍）就一律 **100 点** |
| **代价** | 质料守恒：撑得住才换得成，撑不住就是爆炸 + 等量残渣（正片木棍→钻石块） |

**为什么"本体"必须真换**：在 Minecraft 里"能不能合成"由 **Item** 决定（配方按 Item 匹配，组件救不了），
"能不能放置"也由本体决定（是不是 `BlockItem`）。只搬组件的话，石头永远进不了钻石的配方。
既然本体必须换，"外观不变"就只能靠客户端在渲染时把模型换回源物品——
见 `mixin/NamedItemAppearanceMixin`（1.21.1 里所有物品模型解析都收敛到 `ItemRenderer.getModel`，
手上的、地上的、展示框里的、GUI 格子里的一律走它，一处拦截即全覆盖）。

> 正片里"木铲→下界合金镐"那一幕因此变成：本体是货真价实的下界合金镐（能挖黑曜石、能进配方），
> 看上去还是木铲，耐久按木铲的 59 点算。这比原方案的"耐久 = 名字字数"更贴近"有名字所有的性质"。
...
| 代价 | 规则 | 出处 |
|---|---|---|
| **耐久** | 目标有耐久时取**源物品原本的耐久上限**；源物品没有耐久（石头/木棍）一律 **100 点** | 作者 2026-09-13 定的规则，取代原方案的"耐久 = 名字字数" |
| **质料守恒** | 物品转换：撑得住才换得成，撑不住就爆炸 + 极小残渣 | 【旁白 3:28-3:45】木棍→钻石块 |
| **寿命 = 名字长度** | **实体**：按新身份行动的次数 = 名字字符数 | 【旁白 1:38-2:00】牛多次产蛋后猝死 |
...
- **物品侧的「无 / 地球 / 乱码」**：给"该档案尚未被协会解明"的明确提示，且不消耗命名牌。**没有物品形态的方块名**（水 / 岩浆 / 火）同样落到这里。
...
### 2026-09-13 第二轮改动（作者要求）

1. **去掉解释性文本**：删掉了命名牌的"先过铁砧"用法提示、被命名物品的"命名：X · 剩余 N 次"读数，
   以及 `unknown` / `need_name` 里的解释性括注。玩法靠玩家自己发现，界面不解释。
   （词条在四处同步删除：canonical 基线、工作区 `language_map`、两个 lang 文件、守卫 expectAll）
2. **被命名的东西必须与名称有同样的性质**：物品统一改成"完全转换"——本体真的换成目标物品，
   见 §4.4。自此"石头命名成钻石"得到的是**真钻石**：能合成钻石装备、不能再当方块放下。
   实体的材料名也跟着改了：没有行为的名字（钻石/金锭/牛排…）不再只是"慢慢死掉"，
   而是**析出那个东西**（按质料守恒折算数量，牛 110 ÷ 钻石 30 = 3 颗）。
3. **身份决定能不能被利用**：挤奶/剪毛不再看"它原本是什么"，而看"名字是什么"
   ——牛被命名成"绵羊"剪得出羊毛，命名成"鸡"就挤不出奶（正片原文）。

### 2026-09-13 第三轮改动（作者要求）

1. **外观一律不变**（不只是工具）：新增客户端 mixin `NamedItemAppearanceMixin`，
   在 `ItemRenderer.getModel` 处把模型换成源物品。删掉了原先"工具走组件转让"那一支——
   现在物品只有一条路径：本体换成目标物品 + 客户端画回源物品的外观。
2. **耐久改用源物品的**：目标本身有耐久时，取源物品原本的耐久上限；源物品没有耐久就一律 100 点。
   （"耐久 = 名字字数"作废。）
3. **质料表补了"工具 = 10"这一档**：否则"木铲(1) → 下界合金镐"会被判成质料不足而爆炸，
   正片那一幕就演不出来；同时"木棍(1) → 钻石镐(10)"仍然会炸，物质守恒的直觉保住了。
4. **顺手修了根因**：粉羊与伪云的两个 MCreator 元素定义里 `mobSpawningType` / `modelWidth` /
   `modelHeight` / `trackingRange` 本身就是错的（monster、0.6×1.8、creature、64），
   所以**每次重新生成代码都会被改回去**。已用 `tools/structure-fix/fix_entity_element_defaults.py`
   把元素改成正确值（creature + 0.9×1.3；ambient + 128），从此守卫那两条不再需要反复还原。

### 一次真实的 MCreator 重生成（守卫拦下了）

第二轮改动期间 MCreator 重新生成了代码（`McanomalyarchivesModEntities.java` 与
`McanomalyarchivesModItems.java` 的时间戳同为 11:41:58）：
- **粉羊**被打回 `MobCategory.MONSTER` + `sized(0.6f, 1.8f)`、**伪云**被打回 `CREATURE` + 跟踪距离 64
  → 守卫 `check` 报出 2 处 DRIFT，`apply` 一键还原，39 项恢复全绿。
- **新元素没被吃掉**：`BAIBIAN_NAME_TAG` 的声明与注册行、两个 `NameTag*.init()` 调用都还在
  ——因为元素已挂进 workspace（`locked_code`）、词条已进 `language_map`、init 写在 user code block 里。
  这三处正是"根因修复"，所以这次重生成没有造成任何新损失。
