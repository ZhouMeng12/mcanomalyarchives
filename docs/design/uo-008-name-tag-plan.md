...
**但物品要分两支**（作者 2026-09-13 明确要求："被命名的东西需要与名称有同样的性质"）：

| 目标 | 做法 | 结果 |
|---|---|---|
| **工具 / 护甲** | 组件转让（外观不变） | 正片的木铲→下界合金镐：外形还是木铲、采集能力是下界合金的、耐久 = 名字字数 |
| **其它物品** | **完全转换**（真的换成那个物品） | 石头命名成"钻石"之后**就是钻石**：能合成钻石装备、能进信标、**而且不能再当方块放下去** |

为什么"其它物品"必须真换而不是搬组件：在 Minecraft 里"能不能合成"由**物品本体**决定
（配方按 Item 匹配，组件救不了），"能不能放置"也由物品本体决定（是不是 BlockItem）。
只搬组件的话，石头永远进不了钻石的配方。真换掉之后，"有名字所指事物的全部性质"就自动成立
——耐久、能否放置、能否合成、能否当燃料、能不能吃，一条都不用枚举。

代价因此也从"耐久 = 名字字数"改成**质料守恒**：撑得住才换得成，撑不住就是爆炸（正片木棍→钻石块）。
...
| 代价 | 规则 | 出处 |
|---|---|---|
| **寿命 = 名字长度** | **工具/护甲**：耐久上限 = 名字字符数（"下界合金镐"5 字 → 5 次）；实体：按新身份行动的次数 = 名字字符数 | 【旁白 2:34-2:37】木铲 5 次后损毁 |
| **质料守恒** | 物品做目标的"完全转换"：撑得住才换得成，撑不住就爆炸 + 极小残渣 | 【旁白 3:28-3:45】木棍→钻石块 |
...
2. **被命名的东西必须与名称有同样的性质**：物品路径拆成"工具/护甲 → 组件转让"与"其它 → 完全转换"两支，
   详见 §4.4。自此"石头命名成钻石"得到的是**真钻石**——能合成钻石装备、不能再当方块放下。
