...
public abstract class PlayerAnimationMixin<T extends LivingEntity> {
	...
	public void setupPivot(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		if (master == null)
			master = "mcanomalyarchives";
		if (!master.equals("mcanomalyarchives"))
			return;
		...
		if (entityIn instanceof Player player_)
			...
			return;
		McanomalyarchivesModPlayerAnimationAPI.PlayerAnimation animation = McanomalyarchivesModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null)
			return;
	...
	public void setupAnim(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		if (ageInTicks <= 0)
			return;
		if (!master.equals("mcanomalyarchives")) {
			if (!McanomalyarchivesModPlayerAnimationAPI.animations.isEmpty())
				McanomalyarchivesModPlayerAnimationAPI.animations.clear();
			return;
		}
		PlayerModel<T> model = (PlayerModel<T>) (Object) this;
		Player player = null;
		...
		if (data.getBoolean("ResetPlayerAnimation")) {
			...
			McanomalyarchivesModPlayerAnimationAPI.active_animations.put(player, null);
		}
		...
		McanomalyarchivesModPlayerAnimationAPI.PlayerAnimation animation = McanomalyarchivesModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null) {
			animation = McanomalyarchivesModPlayerAnimationAPI.animations.get(playingAnimation);
			if (animation == null) {
				McanomalyarchivesMod.LOGGER.info("Attepted to play null animation " + playingAnimation + ", did animations fail to load?");
				return;
			}
			McanomalyarchivesModPlayerAnimationAPI.active_animations.put(player, animation);
		}
		...
		for (Map.Entry<String, McanomalyarchivesModPlayerAnimationAPI.PlayerBone> entry : animation.bones.entrySet()) {
			String boneName = entry.getKey();
			McanomalyarchivesModPlayerAnimationAPI.PlayerBone bone = entry.getValue();
			ModelPart modelPart = getModelPart(model, boneName);
			if (modelPart == null)
				continue;
			// Apply rotation
			Vec3 rotation = McanomalyarchivesModPlayerAnimationAPI.PlayerBone.interpolate(bone.rotations, animationProgress, player);
			if (rotation != null) {
			...
			Vec3 position = McanomalyarchivesModPlayerAnimationAPI.PlayerBone.interpolate(bone.positions, animationProgress, player);
			if (position != null) {
			...
			// Apply scale
			Vec3 scale = McanomalyarchivesModPlayerAnimationAPI.PlayerBone.interpolate(bone.scales, animationProgress, player);
			if (scale != null) {
